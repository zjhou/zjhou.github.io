...
const compose = (nodeName, isLeafStr, opt, ...trees) => {
    let nameArr = trees.map(tree => (isLeafStr ? addHBranch(tree, opt.branchLen) : tree));
...
const genAsciiTree = (tree, option) => {
    ...
    return genTree(tree, opt).replace(
        '_'.repeat(opt.branchLen),
        ' '.repeat(opt.branchLen),
    );
};
